...
final class JournalReferenceTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $reference = new JournalReference($authors = [new PersonAuthor(new Person('preferred name', 'index name'))],
            false, 'article title', new Place(null, null, ['journal']));
    ...
    {
        $with = new JournalReference([new PersonAuthor(new Person('preferred name', 'index name'))], true,
            'article title', new Place(null, null, ['journal']));
        $withOut = new JournalReference([new PersonAuthor(new Person('preferred name', 'index name'))], false,
            'article title', new Place(null, null, ['journal']));
    ...
    {
        $reference = new JournalReference([new PersonAuthor(new Person('preferred name', 'index name'))], false,
            'article title', $journal = new Place(null, null, ['journal']));
    ...
    {
        $with = new JournalReference([new PersonAuthor(new Person('preferred name', 'index name'))], false,
            'article title', new Place(null, null, ['journal']), 'volume');
        $withOut = new JournalReference([new PersonAuthor(new Person('preferred name', 'index name'))], false,
            'article title', new Place(null, null, ['journal']));
    ...
    {
        $with = new JournalReference([new PersonAuthor(new Person('preferred name', 'index name'))], false,
            'article title', new Place(null, null, ['journal']), null, '10.1000/182');
        $withOut = new JournalReference([new PersonAuthor(new Person('preferred name', 'index name'))], false,
            'article title', new Place(null, null, ['journal']));
    ...
    {
        $with = new JournalReference([new PersonAuthor(new Person('preferred name', 'index name'))], false,
            'article title', new Place(null, null, ['journal']), null, null, 18183754);
        $withOut = new JournalReference([new PersonAuthor(new Person('preferred name', 'index name'))], false,
            'article title', new Place(null, null, ['journal']));
